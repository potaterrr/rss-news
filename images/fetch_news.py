...
# GitHub Actions provides GITHUB_REPOSITORY automatically (e.g. "potaterrr/your-repo")
GITHUB_REPO = os.getenv("GITHUB_REPOSITORY", "potaterrr/potaterrr")
GITHUB_BRANCH = "main"
IMAGE_DIR = "images"
...
def get_random_repo_image():
    """Scans the local images directory, picks a random file, and returns its raw GitHub URL."""
    if not os.path.exists(IMAGE_DIR):
        os.makedirs(IMAGE_DIR, exist_ok=True)
        logging.warning("Images directory did not exist. Created it. Using fallback image.")
    ...
    # Build direct raw GitHub URL
    raw_url = f"https://raw.githubusercontent.com/{GITHUB_REPO}/{GITHUB_BRANCH}/{IMAGE_DIR}/{chosen_file}"
    logging.info(f"Selected random repo image: {chosen_file} -> {raw_url}")
    return raw_url
...
def main():
    ...
    # 2. Build digest, pick random local image, and send payload to Make.com
    if digest_items:
        logging.info(f"Found {len(digest_items)} new matching articles. Preparing payload...")
        
        combined_digest = "\n\n".join(digest_items)
        random_banner = get_random_repo_image()
        ...
        try:
            ...
            if response.status_code in [200, 201, 204]:
                logging.info("Successfully delivered digest and repo image to Make.com.")
